...
fn test_new_produces_builder_with_bind_addr_happy() {
    ...
    assert_eq!(s.bind_addr(), "10.0.0.1:9090", "builder must pass bind address to server");
}
...
fn test_new_builder_has_no_reflection_by_default_error() {
    // @covers: new
    let s = builder().build();
    assert!(!s.is_reflection_enabled(), "default new() builder must produce server with reflection off");
    assert!(!s.is_unauthenticated_allowed(), "default new() builder must produce server that requires auth");
}
...
fn test_new_builder_has_nonzero_max_message_size_edge() {
    // @covers: new
    let s = builder().build();
    assert_ne!(s.max_message_size(), 0, "default builder must produce server with nonzero message size limit");
    assert_ne!(s.max_concurrent_streams(), 0, "default builder must produce server with nonzero stream limit");
}
...
fn test_build_produces_server_with_all_defaults_happy() {
    // @covers: build
    let s = builder().build();
    assert!(!s.is_reflection_enabled(), "build defaults must have reflection off");
...
fn test_build_without_allow_unauthenticated_defaults_to_false_error() {
    // @covers: build
    let s = builder().build();
    assert!(!s.is_unauthenticated_allowed(), "build without allow_unauthenticated must default to false");
}
...
fn test_build_with_all_options_propagated_edge() {
    ...
    assert!(s.is_unauthenticated_allowed(), "build must propagate allow_unauthenticated");
    assert!(s.is_reflection_enabled(), "build must propagate enable_reflection");
    assert_eq!(s.max_message_size(), 4096, "build must propagate with_max_message_size");
    assert!(matches!(s.compression_mode(), CompressionMode::Gzip), "build must propagate with_compression");
}
...
fn test_with_max_message_size_propagated_to_server_happy() {
    ...
    assert_eq!(s.max_message_size(), 8192, "with_max_message_size must propagate to built server");
}
...
fn test_with_max_message_size_overrides_default_error() {
    ...
    assert_ne!(default_s.max_message_size(), custom_s.max_message_size(), "overridden size must differ from default");
...
fn test_with_max_concurrent_streams_overrides_default_error() {
    ...
    assert_ne!(default_s.max_concurrent_streams(), custom_s.max_concurrent_streams());
...
fn test_with_max_concurrent_streams_minimum_one_edge() {
    ...
    assert_eq!(s.max_concurrent_streams(), 1, "minimum 1 stream must be propagated");
...
fn test_with_tls_propagated_to_server_happy() {
    ...
    assert!(s.tls_config().is_some(), "with_tls must be propagated to built server");
...
fn test_with_tls_default_produces_no_tls_error() {
    // @covers: with_tls
    let s = builder().build();
    assert!(s.tls_config().is_none(), "builder without with_tls must produce server with no TLS");
}
...
fn test_with_tls_overwrite_propagated_edge() {
    ...
    assert!(s.tls_config().is_some(), "second with_tls must be propagated");
...
fn test_allow_unauthenticated_propagated_to_server_happy() {
    ...
    assert!(s.is_unauthenticated_allowed(), "allow_unauthenticated must be propagated to built server");
}
...
fn test_allow_unauthenticated_default_is_false_error() {
    // @covers: allow_unauthenticated
    let s = builder().build();
    assert!(!s.is_unauthenticated_allowed(), "without allow_unauthenticated, server must require auth");
}

#[test]
fn test_allow_unauthenticated_survives_other_options_edge() {
    // @covers: allow_unauthenticated
    let s = builder().allow_unauthenticated().enable_reflection().with_max_message_size(512).build();
    assert!(s.is_unauthenticated_allowed(), "allow_unauthenticated must survive other builder calls");
...
fn test_enable_reflection_propagated_to_server_happy() {
    ...
    assert!(s.is_reflection_enabled(), "enable_reflection must be propagated to built server");
}
...
fn test_enable_reflection_default_is_false_error() {
    // @covers: enable_reflection
    let s = builder().build();
    assert!(!s.is_reflection_enabled(), "without enable_reflection, server must have reflection off");
}
...
fn test_enable_reflection_with_tls_edge() {
    ...
    assert!(s.is_reflection_enabled(), "reflection must survive chaining with TLS");
...
fn test_with_compression_propagated_to_server_happy() {
    ...
    assert!(matches!(s.compression_mode(), CompressionMode::Gzip), "compression must be propagated to built server");
}
...
fn test_with_compression_default_is_none_error() {
    // @covers: with_compression
    let s = builder().build();
    assert!(matches!(s.compression_mode(), CompressionMode::None), "default server must have no compression");
}
...
fn test_with_compression_overwrite_propagated_edge() {
    ...
    let s = builder()
        ...
        .build();
    assert!(matches!(s.compression_mode(), CompressionMode::None), "last compression write must win");
}
...
fn test_with_interceptors_propagated_to_server_happy() {
    ...
    assert!(!s.interceptor_chain().contains_authorization(), "interceptors must be propagated to built server");
}
...
fn test_with_interceptors_overwrite_propagated_edge() {
    ...
    let s = builder()
        ...
        .with_interceptors(GrpcIngressInterceptorChain::new())
        .build();
    assert!(!s.interceptor_chain().contains_authorization(), "second interceptors write must be propagated");
}
...
fn test_with_audit_sink_propagated_to_server_happy() {
    ...
    assert_eq!(*count.lock().unwrap(), 1, "audit sink must be propagated to built server");
}
...
fn test_with_audit_sink_default_does_not_panic_error() {
    ...
    s.audit_sink_ref().record(AuditEvent {
        timestamp: std::time::SystemTime::UNIX_EPOCH,
        method: "/noop".into(), identity: None,
        status: GrpcStatusCode::Ok, duration_ms: 0,
    });
    assert!(!s.is_reflection_enabled(), "default audit sink must not affect other server state");
}
