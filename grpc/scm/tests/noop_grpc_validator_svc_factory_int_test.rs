...
fn test_create_returns_arc_with_strong_count_one_happy() {
    ...
    assert_eq!(Arc::strong_count(&arc), 1, "fresh Arc must have exactly one strong reference");
}
...
fn test_create_clone_increments_strong_count_error() {
    ...
    assert_eq!(Arc::strong_count(&arc), 2, "clone must increment strong count to 2");
}
...
fn test_create_two_calls_produce_independent_instances_edge() {
    ...
    assert!(!Arc::ptr_eq(&a, &b), "separate create() calls must produce distinct instances");
}
