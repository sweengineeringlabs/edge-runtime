...
fn test_default_factory_returns_factory_instance_happy() {
    ...
    assert!(
        ...
        "default_factory produces a factory that generates valid task IDs"
    );
}
...
fn test_default_factory_is_callable_multiple_times_edge() {
    ...
    assert_ne!(
        f1.new_task_id(),
        f2.new_task_id(),
        "consecutive task IDs from multiple factory instances must be unique"
    );
}
...
fn test_default_factory_type_matches_expected_error() {
    ...
    assert!(
        !id.to_string().is_empty(),
        "TaskQueueFactory contract: factory produces valid task IDs"
    );
}
...
fn test_new_task_id_is_callable_on_factory_instance_edge() {
    ...
    assert!(
        !id.to_string().is_empty(),
        "new_task_id produces a non-empty task ID string"
    );
}
...
fn test_build_in_memory_returns_queue_happy() {
    ...
    assert!(
        std::mem::size_of_val(&queue) > 0,
        "build_in_memory must return a non-ZST TaskQueue"
    );
}
...
fn test_build_in_memory_queue_is_send_and_sync_error() {
    ...
    assert!(
        std::mem::size_of::<swe_edge_runtime_message_broker::InMemoryTaskQueue>() > 0,
        "InMemoryTaskQueue is a non-ZST type that compiles as Send + Sync"
    );
}
