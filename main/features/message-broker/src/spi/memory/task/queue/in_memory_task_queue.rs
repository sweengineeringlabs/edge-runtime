...
/// Capacity of the task queue channel — inherited from core configuration.
...
impl TaskQueue for InMemoryTaskQueue {
    fn enqueue(&self, task: Task) -> BoxFuture<'_, Result<(), QueueError>> {
        ...
        Box::pin(async move {
            if task.payload.len() > crate::core::task::MAX_TASK_PAYLOAD_BYTES {
                return Err(QueueError::Enqueue(format!(
                    "payload exceeds maximum size of {} bytes",
                    crate::core::task::MAX_TASK_PAYLOAD_BYTES
                )));
            }
