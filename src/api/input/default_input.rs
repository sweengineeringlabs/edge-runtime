...
impl DefaultInput {
    ...
    pub fn new_grpc(grpc: Arc<dyn GrpcInbound>) -> Self { Self { http: None, grpc: Some(grpc) } }
    /// Construct with no transports configured.
    pub fn empty() -> Self { Self { http: None, grpc: None } }
    /// Add (or replace) the HTTP transport.
    pub fn with_http(mut self, http: Arc<dyn HttpInbound>) -> Self { self.http = Some(http); self }
