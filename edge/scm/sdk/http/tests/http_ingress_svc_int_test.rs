...
fn test_create_noop_http_ingress_independent_instances_edge() {
    ...
    let r1 = block_on(i1.handle(&req, edge_domain::SecurityContext::unauthenticated())).unwrap();
    let r2 = block_on(i2.handle(&req, edge_domain::SecurityContext::unauthenticated())).unwrap();
